...
# json 파일이 위치한 경로를 값으로 줘야 합니다.(현재 같은 위치) 
json_file_path = "./my-project-as608tosheet-accf18336cc4.json"
gc = gspread.service_account(json_file_path)
spreadsheet_url = "https://docs.google.com/spreadsheets/d/1mrTmu0KECyeYIiWiqO3wbza-4FTL8qqoKZR7G1ogEWo/edit#gid=0"
...
def enroll_finger(location):
    ...
    if i == adafruit_fingerprint.OK:
        print("Stored")
        a = input("이름과 학년을 입력하시오 ex)소희:1 >>>")   ### add
        f = open("./info.txt",'a')                       ### add
        f.write(str(location)+':'+a+'\n')                ### add
    else:
        ...
        if get_fingerprint():
            ...
            with open('./info.txt','r') as fp:
                ...
                for i in range(len(file_line)):
                    if finger.finger_id == int(file_line[i][0]):
                        file_line_last = file_line[i].split(':')
                        if int(file_line_last[2].rstrip('\n')) == 1:
                            worksheet=doc.worksheet("1학년")
                            xy = 'a'+str(finger.finger_id)
                            worksheet.update(xy,file_line_last[1])
                        elif int(file_line_last[2].rstrip('\n')) == 2:
                            worksheet=doc.worksheet("2학년")
                            xy = 'a'+str(finger.finger_id)
                            worksheet.update(xy,file_line_last[1])
                        elif int(file_line_last[2].rstrip('\n')) == 3:
                            worksheet=doc.worksheet("3학년")
                            xy = 'a'+str(finger.finger_id)
                            worksheet.update(xy,file_line_last[1]) 
                        else:
                            print('nothing')
                        ...
                        '''
                        with open("attend.txt", "a") as f:
                            tempStr = str(finger.finger_id) +' ' + file_line_last[1] + ' ' + file_line_last[2].rstrip('\n')+"\n"
                            f.write(tempStr)
                        '''
        else:
